...
import com.jawnz.gate.config.KafkaSseConsumer;
import com.jawnz.gate.config.KafkaSseProducer;
...
@RestController
@RequestMapping("/api/jawnzgate-kafka")
public class JawnzgateKafkaResource {

    private final Logger log = LoggerFactory.getLogger(JawnzgateKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public JawnzgateKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
